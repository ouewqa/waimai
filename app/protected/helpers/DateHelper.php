...
class DateHelper
{
     ...
     * 将农历转换为公历
     * @param 农历 $year -年
     * @param 农历 $month -月，闰月处理：例如如果当年闰五月，那么第二个五月就传六月，相当于农历有13个月，只是有的时候第13个月的天数为0
     * @param 农历 $date -日
     * @return array
     */
     ...
     * @param $cmonth
     * @param $cdate
     * @param 农历正月对应的公历月份 $dmonth
     * @param 农历初一对应的公历天数 $ddate
     * @return float
     */
     ...
     * @return string
     */
    public static function getJieQi($_year, $month, $day)
    {
     ...
     * @param $dateline
     * @param bool $nl_info false　即为节目数组的时间就是农历的时间　
     * @param int $config
     * @return string
     */
    public static function getFestival($dateline = false, $nl_info = false, $config = 1)
    {
        if (preg_match('/\d{9,}/', $dateline)) {
            $today = date('Y-m-d', $dateline);
        } else if (strpos($dateline, '-') !== false) {
            $today = $dateline;
        } else {
            $today = date('Y-m-d');
        }
        ...
        if (!$nl_info) {
            $nl_info = self::convertSolarToLunar($y, intval($m), intval($d));
        }
