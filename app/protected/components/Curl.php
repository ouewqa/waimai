...
/**
 * Class Curl
 * referer: https://github.com/ouewqa/php-curl-class
 */
#namespace Curl;
...
class Curl
{
    ...
    public function setCookies($value)
    {
        if (is_array($value)) {
            ...
            $cookies = http_build_query($this->cookies, '', '; ', PHP_QUERY_RFC3986);
        } else {
            $cookies = $value;
        }
    ...
    protected function exec($_ch = null)
    {
        ...
        #字符集转换
        if (is_string($ch->response) && preg_match('/charset=(.+?)\"/is', $ch->response, $charset) && strtolower($charset[1]) != 'utf-8') {
            $ch->response = mb_convert_encoding($ch->response, 'utf-8', $charset[1]);
        }
